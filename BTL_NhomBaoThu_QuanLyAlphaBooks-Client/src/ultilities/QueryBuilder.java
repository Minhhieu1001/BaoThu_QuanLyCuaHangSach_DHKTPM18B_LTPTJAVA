...
public class QueryBuilder {
	...
	public static enum Enum_DataType {
		STRING, INTEGER, DOUBLE, FLOAT, DATE, TIMESTAMP, BOOLEAN
	}
	...
	/**
	 * Tạo query builder, truyền vào một chuỗi query. Hiện đang beta, chỉ có thể tạo
	 * query select
	 * 
	 * @param query  <b>SELECT ... FROM ...? .....</b> <br>
	 *               EX original query: <b>SELECT * FROM NhaSach WHERE id = ? ORDER
	 *               BY id DESC</b> <br>
	 *               Write it into: <b>SELECT * FROM NhaSach ? ORDER BY id DESC.
	 *               </b>
	 * @param params List<Object[]>. Object[] needs follow this {datatype, colname,
	 *               condition, value}
	 */
	 ...
	 *              EX original query: <b>SELECT * FROM NhaSach WHERE id = ? ORDER
	 *              BY id DESC</b> <br>
	 *              Write it into: <b>SELECT * FROM NhaSach ? ORDER BY id DESC. </b>
	 */
	...
	/**
	 * Thêm parameter vào query <br>
	 * Hỗ trợ toán tử '<', '>', '=', '>=', '<=' <br>
	 * Kiểu chuỗi có thêm %. Ví dụ query '%a' sẽ được để ở condition có dạng '%a'.
	 * <br>
	 * Nếu tìm %a + chuỗi người dùng nhập, bạn sẽ có '%a?'
	 * 
	 */
	public void addParameter(Enum_DataType dataType, String columnName, String condition, Object value)
			throws Exception {
		...
		try {
			for (Object[] x : params) {
				...
				if ((Enum_DataType) x[0] == Enum_DataType.STRING && tempCondition.contains("%")) {
					...
					paramsQuery += (tempStr.charAt(0) == '?') ? "CONCAT(?" : "CONCAT('" + tempStr.charAt(0) + "'";
					for (int j = 1; j < tempCondition.length(); j++) {
						paramsQuery += (tempStr.charAt(j) == '?') ? ",?" : ",'" + tempStr.charAt(j) + "'";
					}
					paramsQuery += ")";
				} else
	...
	/**
	 * Tự động set parameters, build theo dữ liệu lấy từ object <br>
	 * Thay vì phải tự kiểm tra dữ liệu có tồn tại hay không để set thì chức năng
	 * này sẽ làm thay bạn
	 * 
	 * @param conn
